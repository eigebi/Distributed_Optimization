...
class problem_generator:
    def __init__(self, num_var, num_agent, num_con, arg):
        ...
        self.C = np.random.randint(2,5,self.num_con)
        ...
        temp[-2] += temp[-1]
        self.con_assignment = np.argsort(temp[:,:-1])[:,-1]
...
if __name__ == '__main__':
    ...
    ub = np.concat([problems.local_probs[i].ub for i in range(N)])
    M_g = 1
    ...
    #latest_x = np.zeros((N,num_var), dtype=np.float32)
    ...
    for t in range(500):
        ...
        delay[id_not_done==1] += 1
        if np.max(delay[id_not_done])>=tau:
            continue
        # local update
        for i in np.where(id_not_done==0)[0]:
            delta = dx[i] + r[problems.con_assignment == i] @ problems.A[problems.con_assignment == i]
            latest_x[i] = z - (delta + gamma[i]) / rho
            latest_x[i] = np.clip(latest_x[i], 0, ub)
            gamma[i] = gamma[i] + rho * (latest_x[i] - z)
        # global update
        eta = 1/(rho*np.pow(k,1/4))

        for i in range(num_var):
            _id = z_graph[i]
            z[i] = (np.sum(gamma[_id==1,i])+rho*np.sum(latest_x[_id==1,i])+beta*z[i])/(beta+np.sum(_id)*rho)
        r = np.maximum((problems.A @ z - problems.C + r ) / (eta*theta + 1), 0)
        k += 1
        for i in np.where(id_not_done==0)[0]:
            dx[i] = problems.gradient_x(i==0, z)
            delay[i] = 0
        id_not_done = np.ones(N, dtype=np.int32)
        obj = problems.obj(z)
        print(obj, problems.A @ z - problems.C,r,z,k)
