...
if __name__ == "__main__":
    ...
    for k in range(20):
        r = np.zeros(5, dtype=np.float32)  # Initialize r (lambda)
        reserved_r = r
        z = np.array([1.5, 0.5], dtype=np.float32)  # Example z(k) values
        gamma = [np.zeros([i], dtype=np.float32) for i in len_x]  # Initialize gamma
        ...
        for f in range(1000):
            for j in range(20):
                ...
                #grad_x = torch.tensor(jac[0](z[0]) + (-r[0]+r[2] + gamma[0] + rho * (x[0].numpy() - z[0])), dtype=torch.float32)
            ...
            # update lambda, every n iterations (when x update is stable)
