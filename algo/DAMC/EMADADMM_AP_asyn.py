        ...
        for test in range(100):
            ...
            for t in range(10000):
                ...
                if local_update:
                    ...
                    obj_t.append(np.abs(res.fun-AP.obj(temp))/np.abs(res.fun))  
                else:
                    obj_t.append(np.abs(res.fun-AP.obj(z))/np.abs(res.fun))
                ...
                if np.any(id_not_done==1) and np.max(delay[np.where(id_not_done==1)])>=_tau:
                    continue
            ...
            acc_t.append(np.array(obj_t))
            inf.append(np.array(infeasibility_t))
            grad_gap.append(np.array(stationary_gap_t))
        np.save("acc_dz_"+str(_tau)+".npy", np.array(acc,dtype=object))
        np.save("acc_dz_t_"+str(_tau)+".npy", np.array(acc_t))

        #np.save("inf_dz.npy", np.array(inf,dtype=object))
        #np.save("grad_gap_dz.npy", np.array(grad_gap,dtype=object))
