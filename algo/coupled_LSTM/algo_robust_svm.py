...
class DMCSolver:
    def __init__(self, env,cfg):
        ...
        self.z = np.ones(self.K)*1  # 全局w向量化存储
    ...
    def solve(self, max_iter=500, rho=5000, theta=0.01, eta_c = 1):
        ...
        gamma_xi = self.gamma_xi.copy()  # xi multiplier 向量化存储
        ...
        for t in range(max_iter):
            ...
            for i in range(self.N):
                agent = self.node[i]
                g_w = agent.grad_w(z, self.agent_xi[i])
                g_xi = agent.grad_xi(z, self.agent_xi[i])
                constraints = agent.constraints(z, self.agent_xi[i])
                J_w = agent.jacobian_w(z, self.agent_xi[i])
                J_xi = agent.jacobian_xi(z, self.agent_xi[i])
                ...
                j_temp += np.sum((dw+gamma[i])**2+np.sum(dxi+self.gamma_xi[i])**2)

                latest_w = z - (dw + gamma[i]) / rho
                latest_xi = self.agent_xi[i] - (dxi+gamma_xi[i]) / rho
                ...
                self.gamma_xi[i] += rho * (latest_xi - self.agent_xi[i])
                self.agent_xi[i] = (beta * self.agent_xi[i] +rho*latest_xi[i] + gamma_xi[i]) / (rho + beta)
                self.agent_xi[i] = np.maximum(latest_xi, 0.0) 
                x[i] = latest_w
...
if __name__ == "__main__":
    ...
    iter_num = 500
    g_1 = algo.solve(max_iter=iter_num, rho = 0.01, theta = 0.001, eta_c = 1)
    g_2 = algo.solve(max_iter=iter_num, rho = 0.01, theta = 0.001, eta_c = 3)
    g_3 = algo.solve(max_iter=iter_num, rho = 0.01, theta = 0.001, eta_c = 5)
    g_4 = algo.solve(max_iter=iter_num, rho = 0.01, theta = 0.001, eta_c = 7)
